...
ambr::p2p::CSHA256::CSHA256() : bytes(0)
{
    sha256::Initialize(s);
}

ambr::p2p::CSHA256& ambr::p2p::CSHA256::Write(const unsigned char* data, size_t len)
{
...
void ambr::p2p::CSHA256::Finalize(unsigned char hash[OUTPUT_SIZE])
{
...
ambr::p2p::CSHA256& ambr::p2p::CSHA256::Reset()
{
    bytes = 0;
    sha256::Initialize(s);
    return *this;
...
void ambr::p2p::SHA256D64(unsigned char* out, const unsigned char* in, size_t blocks)
{
    if (TransformD64_8way) {
        while (blocks >= 8) {
