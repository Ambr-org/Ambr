...
/*static std::vector<CAddress> convertSeed6(const std::vector<SeedSpec6> &vSeedsIn)
{
    ...
    for (const auto& seed_in : vSeedsIn) {
        ...
        addr.nTime = GetTime() - ambr::p2p::GetRand(nOneWeek) - nOneWeek;
...
void AdvertiseLocal(CNode *pnode)
{
    if (fListen && pnode->fSuccessfullyConnected)
    {
        ...
        if (IsPeerAddrLocalGood(pnode) && (!addrLocal.IsRoutable() ||
             ambr::p2p::GetRand((GetnScore(addrLocal) > LOCAL_MANUAL) ? 8:2) == 0))
        {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, nMaxIPs, true))
            {
                for (const CNetAddr& ip : vIPs)
                {
                    ...
                    addr.nTime = GetTime() - 3*nOneDay - ambr::p2p::GetRand(4*nOneDay); // use a random age between 3 and 7 days old
                    vAdd.push_back(addr);
                    found++;
                }
...
void CConnman::ProcessOneShot()
{
    ...
    if (grant) {
        OpenNetworkConnection(addr, false, &grant, strDest.c_str(), false);
    }
}
