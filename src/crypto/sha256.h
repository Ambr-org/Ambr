...
#ifndef AMBR_CRYPTO_SHA256_H_
#define AMBR_CRYPTO_SHA256_H_

namespace ambr {
namespace crypto {

// default 1024*1024 
const int32_t kMaxInputBuffer = 1048576;  
...
class SHA256OneByOneHasher {
public:
  SHA256OneByOneHasher() { 
    init(); 
  }
  ...
  void process(T& value){
    process(reinterpret_cast<const uint8_t*>(&value), reinterpret_cast<const uint8_t*>(&value) + sizeof(value));
  }
...
inline void get_hash_hex_string(const SHA256OneByOneHasher& hasher,
                                std::string& hex_str) {
...
inline std::string get_hash_hex_string(const SHA256OneByOneHasher& hasher) {
  std::string hex_str;
...
void hash256(InIter first, InIter last, OutIter first2, OutIter last2) {
  hash256(
    first, last, first2, last2, kMaxInputBuffer);
}
